...
class Coupon(models.Model):
    ...
    code = models.CharField(max_length=100)  # Changed to lowercase for consistency
...
class Order(models.Model):
    ...
    customer = models.ForeignKey(
        user_models.User, on_delete=models.SET_NULL, null=True, blank=True, related_name="customer"
    )
    ...
    payment_method = models.CharField(
        max_length=100, choices=PAYMENT_METHOD, null=True, blank=True,default=None,
    )
...
class OrderItem(models.Model):
    ...
    shipping_service = models.CharField(
        max_length=100, choices=SHIPPING_SERVICE, null=True, blank=True
    )
    ...
    initial_total = models.DecimalField(
        ...
        help_text="Grand Total of all amounts before discounts."
    )
    saved = models.DecimalField(
        max_digits=12, decimal_places=2, default=0.00,
        null=True, blank=True, help_text="Amount saved from discounts."
    )
    coupons = models.ManyToManyField(Coupon, blank=True)
    applied_coupon = models.BooleanField(default=False)
    item_id = ShortUUIDField(length=6, max_length=25, alphabet="1234567890")
    vendor = models.ForeignKey(
        user_models.User, on_delete=models.SET_NULL, null=True, related_name="vendor_order_items"
    )
    date = models.DateTimeField(default=timezone.now)
...
class Review(models.Model):
    ...
    product = models.ForeignKey(
        Product, on_delete=models.SET_NULL, blank=True, null=True, related_name="reviews"
    )
